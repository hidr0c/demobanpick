    ...
    const unsubscribe = subscribeToMessages((message: ChannelMessage) => {
      switch (message.type) {
        ...
        case 'MATCH_NEXT':
          setState(prev => ({
            ...prev,
            currentMatchIndex: Math.min(prev.currentMatchIndex + 1, prev.matchSongs.length - 1)
          }));
          break;
...
export function useGameController() {
  ...
  useEffect(() => {
    const loadPool = async () => {
      ...
      try {
        const res = await fetch(`${poolFile}?t=${Date.now()}`, { cache: 'no-store' });
        if (res.ok) {
  ...
  const triggerRandom = useCallback(() => {
    ...
    const animationPool = getRandomSongs(availablePool, Math.min(60, availablePool.length));
